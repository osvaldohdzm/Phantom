#!/bin/bash
# Phantom — desarrollo nativo con diagnóstico de puertos (HTTPS + hot-reload).
# Igual que start-dev.sh pero pregunta antes de matar procesos en :8000/:3000.
# Uso: ./phantom debug
#      PHANTOM_FORCE_PORTS=1 ./phantom debug
set -e
...
free_port() {
  ...
  case "$answer" in
    s|S|y|Y|si|sí|SI|SÍ) ;;
    *)
      echo "Cancelado. Libera el puerto manualmente o usa PHANTOM_FORCE_PORTS=1 ./phantom debug"
      exit 1
      ;;
  esac
  fi
...
echo "[*] Si un puerto sigue ocupado: PHANTOM_FORCE_PORTS=1 ./phantom debug"
echo ""
