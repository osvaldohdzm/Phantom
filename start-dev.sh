...
free_port() {
  ...
  case "$answer" in
    s|S|y|Y|si|sí|SI|SÍ) ;;
    *)
      echo "Cancelado. Libera el puerto manualmente o usa PHANTOM_FORCE_PORTS=1 ./start-dev.sh"
      exit 1
      ;;
  esac
  fi
...
echo "[*] Si un puerto sigue ocupado: PHANTOM_FORCE_PORTS=1 ./start-dev.sh"
echo ""
