...
const editableColumnsSet = new Set<string>(VULNS_CATALOG_EDITABLE_COLUMNS);
...
function getUpdatesFromBody(body: unknown) {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return [] as Array<[VulnsCatalogEditableColumn, string | null]>;
  }

  const entries = Object.entries(body);
  const validEntries: Array<[VulnsCatalogEditableColumn, string | null]> = [];
  ...
  for (const [key, value] of entries) {
    ...
    if (value === null) {
      validEntries.push([key as VulnsCatalogEditableColumn, null]);
      continue;
    }
