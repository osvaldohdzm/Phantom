...
describe('Baxter HUB PKI desktop-script strategy', () => {
  ...
  it('fills WinRM worker defaults so the client form does not require the SOC editor tab', () => {
    ...
    const fromWrongLocalCa = resolvePkiWorkerConfig(
      JSON.stringify({ host: '10.11.240.88', caName: 'USDFHUBCAI.hub.baxter.com\\HUB-ISSUING-CA' }),
    );
    expect(fromWrongLocalCa.caName).toBe(BAXTER_PKI_DEFAULT_CA);
