...
def apply_operational_catalog_to_finding(
    ...
    *,
    ...
    """Copia campos Esp* del catálogo operativo al hallazgo. Devuelve True si aplicó algo."""
    changed = False
    ...
    field_map: tuple[tuple[str, str, int], ...] = (
        ("descripcion", "EspDescripcionUnificada", 32000),
        ("amenaza_ampliada", "EspAmenazaUnificadaGeneral", 32000),
        ("metodo_deteccion", "EspMetodoDeteccion", 32000),
        ("explicacion_tecnica", "EspExplicacionTecnica", 32000),
        ("referencias", "References", 32000),
    )
    for attr, key, max_len in field_map:
        raw = cat.get(key)
        if not raw:
            continue
...
def sync_finding_from_operational_catalog(
    ...
    *,
    ...
    if not cat:
        return False
    return apply_operational_catalog_to_finding(finding, cat, force=force)
...
def sync_findings_from_operational_catalog(
    ...
    *,
    ...
    for finding in findings:
        try:
            ensure_finding_tool_identity(finding)
            if sync_finding_from_operational_catalog(db, finding, force=force):
                synced += 1
            else:
                skipped += 1
