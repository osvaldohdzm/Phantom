...
def build_env_template(profile_id: DeploymentProfileId) -> str:
    ...
    if profile_id == "docker_postgres":
        return "\n".join(
            [
                ...
                "",
                "DATABASE_URL=postgresql+psycopg2://postgres:postgresql@127.0.0.1:5432/phantom_security_db_dev",
                "REDIS_URL=redis://localhost:6379/0",
                "AUTH_REQUIRED=true",
                "JWT_SECRET=cambia-este-secreto-en-produccion",
                "JWT_EXPIRE_MINUTES=480",
                ...
                "POSTGRES_PORT=5432",
                "POSTGRES_DB=phantom_security_db_dev",
    ...
    return "\n".join(
        [
            ...
            "# Copia a backend/.env en instalación nueva.",
            "",
            f"DATABASE_URL=postgresql+psycopg2://{user}:{password}@127.0.0.1:5432/phantom_security_db_dev",
            "REDIS_URL=redis://localhost:6379/0",
            "AUTH_REQUIRED=true",
            "JWT_SECRET=cambia-este-secreto-en-produccion",
            "JWT_EXPIRE_MINUTES=480",
