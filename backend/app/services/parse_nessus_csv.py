...
from app.services.ingest_common import clamp_title, map_scanner_severity, parse_float_maybe
...
def _get(row: dict[str, str], *candidates: str) -> str:
    ...
    for c in candidates:
        ...
        if key in index and (index[key] or "").strip():
            return index[key].strip()
    return ""


def parse_nessus_csv_bytes(data: bytes, encoding: str = "utf-8-sig") -> list[dict[str, Any]]:
    ...
    for raw in reader:
        ...
        parts = [
            f"[Nessus CSV] Plugin ID: {plugin_id}" if plugin_id else "[Nessus CSV]",
            f"Host: {host}" if host else None,
            f"Puerto: {port}/{proto}" if port or proto else None,
            f"CVE: {cve}" if cve else None,
        ]
        ...
        out.append(
            {
                ...
                "raw_tool_output": raw_blob[:32000],
            }
        )
    return out
